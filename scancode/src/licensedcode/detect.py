...
from scancode.src.licensedcode.whoosh_spans.spans import Span

from scancode.src.textcode import analysis

from scancode.src.licensedcode import index
from scancode.src.licensedcode.models import get_all_rules
from scancode.src.licensedcode import models
...
def get_license_matches(content=None, index=None, perfect=True):
    """
    Yield detected license matches in the file at `location`.
    """
    return index.match(content)


def detect_license(content=None, perfect=True):
    """
    ...
    """
    for match in get_license_matches(content):
...
class LicenseIndex(object):
    ...
    def match(self, content, perfect=True):
        """
        ...
        qdoc = analysis.text_lines(content)
        if DEBUG:
            qdoc = list(qdoc)
