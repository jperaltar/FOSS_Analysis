...
from scancode.src.licensedcode import saneyaml

from scancode.src.commoncode import fileutils
...
def is_general_license(license):
    general_licenses = ['mit', 'gpl', 'apache', 'bsd', 'lgpl', 'mpl',
                        'public', 'cddl', 'ruby', 'artistic', 'proprietary',
                        'intel-bsd-special', 'swig', 'scancode']
    for general_license in general_licenses:
        if license.startswith(general_license):
            return True
    return False
...
def load_licenses(license_dir=licenses_data_dir):
    ...
    for top, _, files in os.walk(license_dir):
        for yfile in files:
            if not is_general_license(yfile):
                continue
